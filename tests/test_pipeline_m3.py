...
def test_nav_pgm_and_osm_labels_tiny(tmp_path: Path) -> None:
    ...
    assert (pkg / "nav" / "map.pgm").is_file()
    assert (pkg / "nav" / "cost.pgm").is_file()
    assert (pkg / "nav" / "map.yaml").is_file()
    ...
    meta = json.loads((pkg / "nav" / "map_meta.json").read_text(encoding="utf-8"))
