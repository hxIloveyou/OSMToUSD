...
Usage:
  python tools/build_mppi_cost_pgm.py \\
    ...
    --output output/ScenePackages/taibei_ue_20260831/nav2/variants/mppi \\
...
def soft_cost_from_free_mask(
    ...
    *,
    ...
    """Nav2-like soft cost: 0 deep in corridor, rise to 252 near edge, 254 off-road.

    Distance is measured in meters via EDT on the obstacle mask (True = obstacle).
    """
    obstacle = ~free
    # EDT(free): for each free pixel, Euclidean distance (px) to nearest obstacle.
    # Min on free is typically ~1 px (never 0). Half-pixel shift approximates
    # distance to the free/obstacle interface so edge cells get soft cost.
    ...
    # Outside free → already lethal.
    # Inside: if dist >= inscribed → 0; else linear 1..252 as dist → 0.
    ...
    edge = ~deep
    if np.any(edge):
        # d=0 → 252, d=insc → 0 (but edge is d < insc)
        t = np.clip(d[edge] / insc, 0.0, 1.0)
        # near obstacle (t~0) → high cost; near inscribed (t~1) → low
        band[edge] = np.clip(
